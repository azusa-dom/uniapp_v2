...
// MARK: - 学业总览卡片（使用已完成课程数据）
struct AcademicOverviewCard: View {
    @EnvironmentObject var loc: LocalizationService
    
    // 只显示已完成的课程成绩（不包括正在上的课程）
    private let completedCourses: [CourseSummary] = [
        .init(name: "医疗高级机器学习", code: "CHME0017", grade: 85, semester: "上学期"),
        .init(name: "数据科学流行病学", code: "CHME0008", grade: 69, semester: "上学期")
    ]
    
    // 当前正在上的课程数量
    private let ongoingCoursesCount = 5
    
    var averageGrade: Double {
        guard !completedCourses.isEmpty else { return 0 }
        let total = completedCourses.reduce(0) { $0 + $1.grade }
        return Double(total) / Double(completedCourses.count)
    }
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                HStack(alignment: .center, spacing: 16) {
                    Text(String(format: "%.1f", averageGrade))
                        ...
                        .foregroundColor(gradeColor(for: Int(averageGrade)))
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(degreeLevel(for: averageGrade))
                        ...
                        Text("基于 \(completedCourses.count) 门已完成课程")
                            .font(.system(size: 10))
                            .foregroundColor(.secondary)
                            .padding(.top, 2)
                    }
    ...
    private func degreeLevel(for average: Double) -> String {
        switch Int(average) {
        case 70...100: return "🏆 一等学位水平"
        case 60..<70: return "⭐ 二等学位水平"
        case 50..<60: return "✓ 及格水平"
        default: return "需要努力"
        }
    }
