...
// MARK: - Data Models (API)

struct UCLAPITimetableEvent: Codable, Identifiable {
    let id = UUID()
    let moduleName: String
    let moduleId: String
    let startTime: String
    let endTime: String
    let location: UCLAPILocation?
...
struct UCLAPITimetableResponse: Codable {
    let ok: Bool
    let timetable: [String: [UCLAPITimetableEvent]]
}
...
class UCLAPIManager {
    ...
    // 获取个人时间表
    func fetchPersonalTimetable() async throws -> [UCLAPITimetableEvent] {
        ...
        let timetableResponse = try JSONDecoder().decode(UCLAPITimetableResponse.self, from: data)
    ...
    // 按模块获取时间表
    func fetchTimetableByModules(_ modules: [String]) async throws -> [UCLAPITimetableEvent] {
        let parameters = [
...
struct TimetableView: View {
    @State private var events: [UCLAPITimetableEvent] = []
